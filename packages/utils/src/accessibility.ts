...
export function announceFinancialChange(
  ...
  const currentFormatted = valueType === 'volume'
    ? currentValue.toLocaleString()
    : currencyFormatter.format(currentValue)

  const previousFormatted = valueType === 'volume'
    ? previousValue.toLocaleString()
    : currencyFormatter.format(previousValue)

  const changeFormatted = valueType === 'volume'
  ...
  const priority = Math.abs(change / previousValue) > 0.05 ? 'assertive' : 'polite'
